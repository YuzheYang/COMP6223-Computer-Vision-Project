...
if __name__ == '__main__':
    from data_loader_submit import *
    from matplotlib import pyplot as plt
    from keras.callbacks import ReduceLROnPlateau

    input_img = Input(shape=(None, None, 3))
    input_sal = Input(shape=(None, None, 3))
    ...
    xl = base_model(input_img)
    xr = base_model(input_sal)
    xcon = Concatenate()([xl, xr])
    xcon = GlobalAveragePooling2D()(xcon)

    # let's add a fully-connected layer
    xcon = Dense(2048, activation='relu')(xcon)
    xcon = Dropout(0.5)(xcon)
    xcon = BatchNormalization()(xcon)

    # and a logistic layer -- let's say we have 200 classes
    predictions = Dense(15, activation='softmax')(xcon)
    # this is the model we will train
    model = Model(inputs=[input_img, input_sal], outputs=predictions)
    ...
    # plot_model(model)
    x_train, y_train, x_test, y_test = training_data_loader()
    sal_xtrain,sal_ytrain, sal_xtest, sal_ytest = saliency_train_data_loader()


    reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.2,
                                  patience=5, min_lr=0.00005)

    history = model.fit([x_train, sal_xtrain],
                        y_train,
                        batch_size=32,
                        verbose=1,
                        epochs=30,
                        validation_data=([x_test, sal_xtest], y_test),
                        callbacks=[reduce_lr])
    #
    plt.figure()
    ...
    plt.legend(['Train Top-1', 'Train Top-5', 'Test Top-1','Test Top-5'], loc='best')
    plt.title('Model accuracy')
    plt.ylabel('Accuracy')
    plt.xlabel('Epoch')
    plt.grid(True)
    plt.savefig('acc-submit.png')
    ...
    plt.xlabel('Epoch')
    plt.grid(True)
    plt.savefig('loss-submit.png')
    model.save('SSRSubmitModel.h5')